...
#   CONTAINER_NAME --name value               (default: agent-host-cc; pass empty to let container auto-name)
...
#   REBUILD        "1" → run build-apple-container.sh ${IMAGE_TAG} before running (default: unset)
...
if [[ "${need_build}" == "1" ]]; then
  ...
  IMAGE_NAME="${IMAGE_NAME}" bash "$(dirname "$0")/build-apple-container.sh" "${IMAGE_TAG}"
fi

# Reclaim the container name if a previous run left one behind. Apple
# `container run` rejects `--name X` when a container called X already exists in
# any state. Inspect → stop → delete is the safe sequence.
if [[ -n "${CONTAINER_NAME}" ]] && container inspect "${CONTAINER_NAME}" >/dev/null 2>&1; then
